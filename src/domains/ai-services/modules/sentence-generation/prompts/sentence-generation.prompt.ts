...
export const sentenceGenerationPrompt: PromptTemplate<GenerateSentenceParams> =
  {
    version: '1.33.0',
    template: `Твоя задача — создать образцовую учебную пару предложений (английское и русское) с подсказкой для изучения иностранного языка, строго следуя параметрам и требованиям ниже.
...
*   **Фокус на Грамматике и Слове**: Грамматическая тема "{{grammarTopic}}" и слово "{{word}}" (в его {{partOfSpeech}}) должны быть ЦЕНТРАЛЬНЫМИ, корректно и ЯВНО продемонстрированными элементами в АНГЛИЙСКОМ предложении. Связь между "{{word}}" и "{{grammarTopic}}" должна быть логичной и осмысленной. Русский эквивалент слова "{{word}}" также должен быть использован корректно в соответствующей части речи в русском предложении.
...
1.  **Анализ Слова, Выбор Значения и Определение Эквивалента**:
    *   Внимательно проанализируй английское слово "{{word}}" ({{partOfSpeech}}), учитывая его возможные значения.
    *   **Выбор релевантного значения**: Из всех возможных значений "{{word}}", выбери ОДНО, которое является наиболее употребимым, понятным и полезным для изучения на уровне {{cefrLevel}}. Это значение также должно хорошо сочетаться с грамматической темой "{{grammarTopic}}" и позволять создать ясный, естественный пример. (Например, для {{word}}: "change" (noun) и {{cefrLevel}}: "A1", значение "сдача (деньги)" или "перемена (изменение планов/погоды)" будут более предпочтительны, чем более абстрактные или редкие значения).
    *   **Определение русского эквивалента для выбранного значения**: Для этого выбранного значения "{{word}}", определи наиболее точный и распространенный русский эквивалент. Этот русский эквивалент:
        *   Должен соответствовать {{partOfSpeech}}.
        *   Должен быть понятен на уровне {{cefrLevel}}.
        *   В идеале, должен наиболее однозначно указывать обратно на "{{word}}" (в выбранном значении) при переводе с русского на английский.
2.  **Английское Предложение**:
    *   Придумай ОРИГИНАЛЬНОЕ английское предложение, которое:
        ...
        *   ЯВНО, КОРРЕКТНО и ПРОСТО демонстрирует грамматику "{{grammarTopic}}", используя слово "{{word}}" в его {{partOfSpeech}}.
...
-   Типы предложений: Варьируй между утвердительными, отрицательными и вопросительными конструкциями в АНГЛИЙСКОМ предложении, если это уместно для грамматической темы и уровня.
-   Начало предложений: Старайся начинать АНГЛИЙСКИЕ предложения с разных членов предложения для разнообразия, если это звучит естественно.
...
-   Лексика: Используй синонимы и разнообразные лексические конструкции в АНГЛИЙСКОМ предложении, соответствующие уровню, чтобы избежать монотонности.
-   Эмоциональная окраска: По возможности, делай предложения позитивными или нейтральными.
-   Наглядность: Пример должен четко иллюстрировать грамматическое правило или функцию слова в АНГЛИЙСКОМ предложении.
